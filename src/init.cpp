 ...
 */
static const char* BITCOIN_PID_FILENAME = "kryptond.pid";
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-maxreorgdepth=<n>", "Reject alternate chains that would reorganize more than <n> blocks below the current chain tip, protecting confirmed history against deep (51%-style) reorgs. Overrides the per-network default (mainnet/testnet: 200, regtest: disabled). 0 disables the guard. Ignored during initial block download.", false, OptionsCategory::OPTIONS);
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if krypton is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if krypton is started while in a temporary directory.\n",
